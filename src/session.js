...
export class GameSession {
  ...
  completeLevel(levelId,stars,score,time,{maxStars}={}) { const old=this.profile.levels[levelId]||{stars:0,bestScore:0,bestTime:null,completed:false,unlocked:true}; const cap=Number.isFinite(maxStars)?Math.max(0,Math.floor(maxStars)):Math.max(old.stars,Math.floor(Number(stars)||0)); const earned=Math.max(0,Math.min(cap,Math.floor(Number(stars)||0))); this.profile.levels[levelId]={stars:Math.max(old.stars,earned),bestScore:Math.max(old.bestScore,score),bestTime:old.bestTime===null?time:Math.min(old.bestTime,time),completed:true,unlocked:true}; this.profile.lifetimeStars=Object.values(this.profile.levels).reduce((sum,item)=>sum+item.stars,0); this.store.save(this.profile); }
}
