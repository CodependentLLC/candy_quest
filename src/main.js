...
const map = new WorldMap(document.querySelector("#world-map"), getWorld("world-01"), game.session, {
  onSelect: levelId => { game.setLevel(getLevel(levelId), levelId); map.hide(); game.paused=false; game.audio?.setPaused(false); game.updatePauseOverlay(); },
  onBack: () => map.show()
});
const openMap = () => { game.paused=true; game.audio?.setPaused(true); map.show(); };
...
document.querySelector("#result-map")?.addEventListener("click",()=>{ game.resultMode=null; game.completed=false; game.gameOver=false; game.updateResultOverlay(); game.paused=true; game.audio?.setPaused(true); map.show(); });
